        ...
        for action_timestep in range(action_repeat):
            ...
            if collision:
                ...
                if action_timestep < action_repeat * .3: #in case prior action caused collision
                    network_params[action_i-1][-1] = collision #share collision result with prior action
                break
        l.append([action_i,collision])
        network_params[action_i]=[sensor_readings[0],sensor_readings[1],sensor_readings[2],sensor_readings[3],sensor_readings[4],action,collision]
            
    
    
    return network_params,l
...
if __name__ == '__main__':
    total_actions = 10000
    result,l = collect_training_data(total_actions)
